...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        static std::vector<ChatCommand> commandTable =
        {
            ...
            { "groupsummon",          rbac::RBAC_PERM_COMMAND_GROUP_SUMMON_PLAYER,          false, &HandleGroupSummonCommand,          "" },
            { "shop",          rbac::RBAC_PERM_COMMAND_STORE,          false, &HandleStoreCommand,          "" },
            { "premium-mailbox",          rbac::RBAC_PERM_COMMAND_STORE,          false, &HandlePremiumMailboxCommand,          "" },
            { "premium-bank",          rbac::RBAC_PERM_COMMAND_STORE,          false, &HandlePremiumBankCommand,          "" },
            { "xp",          rbac::RBAC_PERM_COMMAND_STORE,          false, &HandleModifyXpCommand,          "" },
        };
        return commandTable;
    }
    ...
    static bool HandleModifyXpCommand(ChatHandler* handler, char const* args) {
        ...
        if (killRate < 1) {
            handler->SendSysMessage("You cannot set your xp to 0.");
            handler->SetSentErrorMessage(true);
            return true;
        }

        if (killRate > 12) {
            handler->SendSysMessage("You cannot set your xp higher than 12.");
            handler->SetSentErrorMessage(true);
            return true;
        }

        if (Player* player = handler->GetSession()->GetPlayer())  {
            player->SetCustomXpRate(killRate);
            handler->PSendSysMessage("Your xp are now set to %f! Good game on Tanaris.", killRate);
            return true;
          }
    ...
    static bool havePremium(Player* player) {
        return true;
    }

    static bool HandlePremiumBankCommand(ChatHandler* handler, char const* args) {
        Player* player = handler->GetSession()->GetPlayer();
        if (havePremium(player)) {
            handler->GetSession()->SendShowBank(handler->GetSession()->GetPlayer()->GetGUID());
            return true;
        }
        else {
            handler->PSendSysMessage("You not a premium member.");
            return true;
    ...
    static bool HandlePremiumMailboxCommand(ChatHandler* handler, char const* args) {
        Player* player = handler->GetSession()->GetPlayer();
        if (havePremium(player)) {
    ...
    static bool HandleStoreCommand(ChatHandler* handler, char const* args) {
    ...
    static bool HandleGroupSummonCommand(ChatHandler* handler, char const* args) {
        ...
        if (target->IsInCombat()) {
            handler->PSendSysMessage("You can't use this command in combat.");
            return true;
        }

        if (target->InBattleground()) {
            handler->PSendSysMessage("You can't use this command in battleground.");
            return true;
        }

        if (target->GetMap()->IsDungeon()) {
            handler->PSendSysMessage("You can use this only outside of a dungeon or raid.");
            return true;
        }


        if (!group) {
            handler->PSendSysMessage("You need to be in group to use this commande.");
            return true;
        }
        ...
        if (timeElapsed < 60) {
            handler->PSendSysMessage("This ability isn't ready yet, cooldown %u min(s)", 60 - timeElapsed);
            return true;
        }
        ...
        LoginDatabase.PQuery("UPDATE account SET summon = NOW() WHERE id = %u", target->GetSession()->GetAccountId());
